...
kernel5(dtype *g_idata, dtype *g_odata, unsigned int n)
{
  ...
  unsigned int iid = bid * blockDim.x + threadIdx.x;
  unsigned int tid = threadIdx.x;
  int ni = gridDim.x * blockDim.x;
  scratch[tid]=0;
  while(iid<n){
  scratch[threadIdx.x]+=g_idata[iid];
  iid +=ni;
  }
  __syncthreads ();
  ...
  if(tid<32)
  {
    volatile dtype *temp = scratch;
    if (n>64) temp[tid] += temp[tid+32];
    if (n>32) temp[tid] += temp[tid+16];
    temp[tid]+=temp[tid+8];
    temp[tid]+=temp[tid+4];
    temp[tid]+=temp[tid+2];
    temp[tid]+=temp[tid+1];
